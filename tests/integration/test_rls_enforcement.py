...
class TestPostCutoverWritePolicies:
    ...
    async def test_progress_summary_upsert_admits_editor(
        ...
        async with as_trajan_app(db_session):
            # total_additions / total_deletions declared NOT NULL with a
            # Python-only default on the SQLModel — a raw INSERT that omits
            # them gets NULL from Postgres. List them explicitly.
            await db_session.execute(
                text(
                    "INSERT INTO progress_summary "
                    "(product_id, period, summary_text, "
                    " total_commits, total_contributors, total_additions, total_deletions) "
                    "VALUES (:pid, '7d', 'first narrative', 42, 3, 100, 20)"
                ),
                {"pid": product_id},
            )
            await db_session.flush()
    ...
    async def test_dashboard_shipped_summary_insert_admits_editor(
        ...
        async with as_trajan_app(db_session):
            # total_additions / total_deletions / merged_prs declared NOT NULL
            # with Python-only defaults — same pattern as progress_summary.
            await db_session.execute(
                text(
                    "INSERT INTO dashboard_shipped_summary "
                    "(product_id, period, items, has_significant_changes, "
                    " total_commits, total_additions, total_deletions, merged_prs) "
                    "VALUES (:pid, '7d', '[]'::jsonb, true, 12, 200, 40, 3)"
                ),
                {"pid": product_id},
            )
            await db_session.flush()
    ...
    async def test_users_self_delete_rejects_other(
        ...
        # Verify outside the trajan_app context — under `postgres` (BYPASSRLS)
        # the still-present victim row is visible. Inside the context, the
        # SELECT policies `users_select_own` + `users_select_org_members`
        # would hide the row from `acting` (different owner, no shared org),
        # making a hidden-but-present row indistinguishable from a deleted
        # one. The DELETE above silently filtered to zero rows via its
        # USING clause; this confirms the victim was never actually deleted.
        result = await db_session.execute(
            text("SELECT count(*) FROM users WHERE id = :uid"),
            {"uid": victim.id},
        )
