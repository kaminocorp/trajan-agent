...
class TokenResolver:
    ...
    async def resolve_token_for_org(
        ...
        """Resolve a single token with org context.
        ...
        For listing repos across ALL installations, use
        ``resolve_all_tokens_for_org()`` instead.
        """
    ...
    async def resolve_all_tokens_for_org(
        self, organization_id: UUID, user_id: UUID
    ) -> list[tuple[str, str, str]]:
        """Resolve tokens for ALL active GitHub App installations in an org.

        Returns a list of (token, method, github_account_login) tuples —
        one per active installation. Falls back to the user PAT when no
        App installations are available.

        Used by repo-listing endpoints that need to aggregate repos across
        multiple GitHub accounts connected to the same Trajan org.
        """
        from app.domain import github_app_installation_ops
        ...
        if github_app_auth.is_configured:
            ...
            for installation in installations:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to get App token for installation %s (%s), skipping",
                        installation.installation_id,
                        installation.github_account_login,
                    )
        ...
        # No App installations — fall back to user PAT
        pat_token = await self._try_user_pat(user_id)
        if pat_token:
            return [(pat_token, "pat", "")]
