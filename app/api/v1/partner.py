...
from app.api.deps.org_api_key_auth import require_partner_scope
...
async def get_partner_key(
    api_key: OrgApiKey = Depends(require_partner_scope("partner:read")),
) -> OrgApiKey:
    """Dependency: validate partner scope + enforce rate limit."""
    rate_limiter.check_rate_limit(api_key.id, "partner", PARTNER_READ_LIMIT)
    return api_key
...
async def get_org_pulse(
    ...
    api_key: OrgApiKey = Depends(get_partner_key),
    ...
    # Fetch cached stats + product/repo counts in parallel-safe queries
...
async def get_org_products(
    ...
    # Fetch all products with lead user and repositories eagerly loaded
    products = await product_ops.get_by_organization(db, org_id)
    if not products:
        return ProductPortfolioResponse(products=[], period=period)

    # Batch-fetch progress summaries for all products
...
async def get_org_shipped(
    ...
    # Get product IDs and names for this org
    ...
    for s in summaries:
        ...
        # Merge top contributors across products
    ...
    # Sort contributors by total LOC
...
async def get_org_contributors(
    ...
    # Fetch org-level contributor summary
    ...
    # Build product map for cross-referencing which products each contributor touched
    product_stmt = select(Product.id, Product.name).where(  # type: ignore[call-overload]
        Product.organization_id == org_id
    )
    product_result = await db.execute(product_stmt)
    ...
    # Fetch per-product progress summaries to find contributor→product mapping
    summary_stmt = select(ProgressSummary).where(
        ProgressSummary.product_id.in_(product_ids),  # type: ignore[attr-defined]
        ProgressSummary.period == period,  # type: ignore[arg-type]
    )
    ...
    # Build contributor → products mapping from per-product contributor_summaries
    ...
    # Convert summaries dict to ranked list
    contributor_items: list[ContributorItem] = []
    for name, data in team_summary.summaries.items():
        contributor_items.append(
            ContributorItem(
                name=name,
                avatar_url=None,  # Not stored in team summary — enriched at generation time
...
async def get_org_changelog(
    ...
    # Build query: changelog entries joined with products for org scoping
    stmt = (
    ...
    # Count total (without limit) for pagination context
    count_stmt = (
        select(func.count())
...
async def get_org_narrative(
    ...
    # Fetch team contributor summary (team_summary + per-contributor narratives)
    team_summary = await team_contributor_summary_ops.get_by_org_period(db, org_id, period)

    # Fetch all products + their progress summaries
    product_stmt = select(Product.id, Product.name).where(  # type: ignore[call-overload]
        Product.organization_id == org_id
    )
    product_result = await db.execute(product_stmt)
    ...
    # Batch-fetch progress summaries
    ...
    if product_ids:
        ...
        for ps in summary_result.scalars().all():
            ...
            # Collect contributor names for deduplication across products
    ...
    # Build contributor narratives from team summary
    ...
    if team_summary and team_summary.summaries:
        ...
        # Use the later of the two generated_at timestamps
